...
class _Display_imageState extends State<Display_image> {
  ...
  getPref() async {
    ...
    setState(() {
      ...
      print('DEBUG: [Display_image] base64string loaded: ' + (base64string != null && base64string.length > 30 ? base64string.substring(0, 30) + '...' : (base64string ?? 'null')));
    ...
    convert_image(); // Call convert_image only after loading base64string
    print('**********************1******************');
    print(Final_score);
  ...
  String? _decodeError;
  void convert_image() {
    print('DEBUG: [Display_image] convert_image called with base64string: ' + (base64string != null && base64string.length > 30 ? base64string.substring(0, 30) + '...' : (base64string ?? 'null')));
    try {
      if (base64string == null || base64string == '' || base64string == 'Not Selected') {
        setState(() {
          _decodeError = 'No image data found or image not selected.';
          augustus3 = -1;
        });
        return;
      }
      Timer(Duration(milliseconds: 1), () {
        try {
          ...
          setState(() {
            _decodeError = 'Failed to decode image: '
                + e.toString();
            augustus3 = -1;
          });
        }
      ...
      setState(() {
        _decodeError = 'convert has failed: ' + e.toString();
        augustus3 = -1;
      });
    }
  }
